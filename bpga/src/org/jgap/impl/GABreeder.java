...
import bnppga.BPNNGA;
import bnppga.GetFit;
...
public class GABreeder
  ...
  protected void updateChromosomes(Population a_pop, Configuration a_conf, BPNNGA obj,
			FitnessFunction fitness, int percent,  BufferedWriter output) {
	    GetFit gf = new GetFit();
	    gf.getfitness(a_pop, a_conf, obj, fitness, percent,  output);
	    
	  }

@Override
public Population evolve(Population a_pop, Configuration a_conf, BPNNGA obj,
		FitnessFunction fitness, int percent,  BufferedWriter output) {
	    Population pop = a_pop;
	    int originalPopSize = a_conf.getPopulationSize();
	    boolean monitorActive = a_conf.getMonitor() != null;
	    ...
	    updateChromosomes(a_pop, a_conf, obj,fitness, percent, output);
	    if (monitorActive) {
	      // Monitor that fitness value of chromosomes is being updated.
	      // -----------------------------------------------------------
	      a_conf.getMonitor().event(
